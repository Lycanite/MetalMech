...
public class Hooks {
	...
	// Get Metallurgy Item:
	private String[] metalSetNames = { "MetalMech", "baseSet", "preciousSet", "netherSet", "fantasySet", "enderSet", "utilitySet" };
	private List<Object> metalSets = new ArrayList<Object>();
	public ItemStack getMetallurgyItem(String metalName, String itemType, int amount) {
		try {
			// Methods
			checkMethodHook("getOreInfo", "MetalSet", String.class);
			
			// Invoke Methods
			checkMetalsets();
			for(Object metalSetN : metalSets) {
				Object oreInfo = hookMethod("getOreInfo").invoke(hookClass("MetalSet").cast(metalSetN), new Object[] { metalName });
				if(oreInfo != null) {
					// Item
					if(itemType != "ore" && itemType != "block" && itemType != "brick") {
						Object itemInfo = oreInfo.getClass().getDeclaredField(itemType).get(oreInfo);
						if(itemInfo instanceof Item) return new ItemStack((Item)itemInfo, amount);
					}
					// Block
					else {
						Field blockIDField = oreInfo.getClass().getDeclaredField(itemType + "ID");
						blockIDField.setAccessible(true);
						int blockID = (Integer)blockIDField.get(oreInfo);
						
						Field blockMetaField = oreInfo.getClass().getDeclaredField(itemType + "ID");
						blockMetaField.setAccessible(true);
						int blockMeta = (Integer)blockMetaField.get(oreInfo);
						
						return new ItemStack(blockID, amount, blockMeta);
					}
				}
			}
			
			// Return ItemStack
			System.out.println("An error occurred when trying to get " + metalName + " " + itemType + " from Metallurgy ore info.");
			return null;
		}
		catch(Exception e) {
			System.out.println("[MetalMech] There was a problem reading from the Metallurgy item list:");
			e.printStackTrace();
			return null;
		}
	}
	
	public void checkMetalsets() throws ClassNotFoundException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
		// Return if MetalSets are already loaded.
		if(metalSets.size() > 0) return;
		
		// Get MetalSets
		for(String metalSetName : metalSetNames) {
			if(metalSetName == "MetalMech") metalSets.add(MetalMech.metalSet);
			else metalSets.add(hookClass("MetallurgyMetals").getDeclaredField(metalSetName).get(null));
		}
	}
